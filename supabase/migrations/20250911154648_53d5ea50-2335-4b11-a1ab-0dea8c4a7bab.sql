...
-- Create improved policies using existing security definer functions
...
USING (
  ...
  EXISTS (
    ...
    AND j.recruiter_id = public.get_user_internal_id()
  )
);
...
USING (
  role = 'candidate'::user_role AND 
  public.get_user_role() = 'recruiter'::user_role AND
  EXISTS (
