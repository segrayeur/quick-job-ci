...
const ChatBot = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('chatbot-rag', {
        body: {
  ...
  return <>
      ...
      {/* Chat Window */}
      {isOpen && <Card className="fixed bottom-6 right-6 w-80 h-96 shadow-elevated z-50 border-0 bg-background">
          ...
          <CardContent className="p-0 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {messages.map(message => <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] px-3 py-2 rounded-lg text-sm ${message.isUser ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground'}`}>
                    {message.content}
                  </div>
                </div>)}
            ...
            <div className="border-t p-4">
              <div className="flex space-x-2">
                <Input value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={handleKeyPress} placeholder="Tapez votre message..." disabled={isLoading} className="flex-1" />
                <Button onClick={sendMessage} disabled={isLoading || !inputMessage.trim()} size="sm" className="bg-gradient-primary hover:opacity-90">
